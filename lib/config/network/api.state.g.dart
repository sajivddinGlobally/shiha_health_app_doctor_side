...
class _APIStateNetwork implements APIStateNetwork {
  ...
  Future<LeaveUpdateResModel> leaveUpdate(
    ...
    final _data = <String, dynamic>{};
    _data.addAll(body.toJson());
    final _options = _setStreamType<LeaveUpdateResModel>(
      Options(method: 'PUT', headers: _headers, extra: _extra)
          .compose(
            _dio.options,
            '/api/doctor-leaves/${id}',
    ...
    final _result = await _dio.fetch<Map<String, dynamic>>(_options);
    late LeaveUpdateResModel _value;
    try {
      _value = LeaveUpdateResModel.fromJson(_result.data!);
    } on Object catch (e, s) {
      errorLogger?.logError(e, s, _options, response: _result);
      rethrow;
    }
